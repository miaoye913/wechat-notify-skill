---
name: wechat-notify
description: 给用户微信发通知消息（任务完成/失败/提醒），支持双通道问答与测试例程。当用户要求"跑完/完成后用微信通知我"、"运行测试例程"时使用。默认发到微信 ClawBot 对话，失败时可用服务号兜底（-c wechat）。工具目录 {{TOOL_DIR}}，token 已配置无需用户提供。
---
...
- token 已配置在工具同目录（`token.txt` / `gate.env`）；不要输出/展示 token
